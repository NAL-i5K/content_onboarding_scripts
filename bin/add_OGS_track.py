...
def get_loading_types(gff):
    # type in column 3
    load_as_first = ['pseudogene', 'sequence_alteration']
    loading_types = {
        'first': set(),
        'second': set()
    }
    ...
    for key in gff_model:
        # first-level usually contains one feature type
        first_level_type = gff_model[key][1].pop()

        # first-level -> second-level -> third-level (load as second-level feature)
        if len(gff_model[key]) > 2:
            if first_level_type in load_as_first:
                loading_types['first'].add(first_level_type)
            else:
...
def main(args):
    ...
    load_to_Gene = set(loading_types['second'])
    # load sequence_alteration to the track of Gene
    if 'sequence_alteration' in loading_types['first']:
        load_to_Gene.add('sequence_alteration')
    load_to_Pseudogene = set()
    if 'pseudogene' in loading_types['first']:
        load_to_Pseudogene.add('pseudogene')
    ...
    config_template = "\'{ \"category\": \"Official Gene Set\"}\'"
    config_init = {
    ...
    if not args.trackLabel_gene:
        args.trackLabel_gene = 'OGSv%s_Gene' % (args.release_version)
    template_init['trackLabel'] = args.trackLabel_gene
    if not args.key_gene:
        template_init['key'] = 'OGSv%s Gene' % (args.release_version)
    else:
    ...
    if not args.trackLabel_psudogene:
        args.trackLabel_psudogene = 'OGSv%s_Pseudogene' % (args.release_version)
    template_init['trackLabel'] = args.trackLabel_psudogene
    if not args.key_psudogene:
        template_init['key'] = 'OGSv%s Pseudogene' % (args.release_version)
    else:
        template_init['key'] = args.key_psudogene
    template_init['type'] = ','.join(list(load_to_Pseudogene))
    template_init['trackType'] = 'HTMLFeatures'
    if not args.config:
        config_init['key'] = template_init['key']
...
if __name__ == '__main__':
    ...
    Quick start:
    %(prog)s -path Apollo/bin/flatfile-to-json.pl -gff test.gff -out jbrowse/data/ -organism Leptinotarsa_decemlineata -trackLabel1 lepdec_current_models -release 1.2
    """))
